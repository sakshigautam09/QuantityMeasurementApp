...
namespace QuantityMeasurementRepository
{
    ...
    /// </summary>
    public class QuantityMeasurementCacheRepository : IMeasurementHistoryRepository
    {
        // ── Singleton ────────────────────────────────────────────────────
        ...
        {
            // Repository layer: local cache JSON under QuantityMeasurementRepository/Data/
            // (copied to app output via csproj; see AppContext.BaseDirectory when running Console)
           // Save to the QuantityMeasurementApp project folder (3 levels up from bin/Debug/net8.0)
            var projectDir = Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..", "..", ".."));
            _filePath = Path.Combine(projectDir, "quantity_measurements.json");
