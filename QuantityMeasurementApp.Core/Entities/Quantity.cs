...
namespace QuantityMeasurementApp.Core.Entities
{
    public sealed class Quantity<U>
    {
        ...
        // compare by base-unit value, not by reference ────────────────────────

        public override bool Equals(object? obj)
        {
            if (obj is not Quantity<U> other) return false;
            double thisBase  = ConvertToBaseUnit(unit,       value);
            double otherBase = ConvertToBaseUnit(other.unit, other.value);
        ...
        {
            // Hash on the rounded base value so equal quantities have the same hash
            double baseValue = Math.Round(ConvertToBaseUnit(unit, value), 9);
        ...
        private static double ConvertFromBaseUnit(U u, double v) => u switch
        {
            ...
            TemperatureUnit tu => tu.ConvertFromBaseUnit(v),
            _ => throw new InvalidOperationException("Unsupported unit type")
        };
    }
}